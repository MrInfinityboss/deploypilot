...
$Version = if ($env:DEPLOYPILOT_VERSION) { $env:DEPLOYPILOT_VERSION } else { "v1.0.1" }
...
$ApiUrl = Read-Host "DeployPilot API URL"
$WorkerId = Read-Host "Worker ID"
$WorkerTokenSecure = Read-Host "Worker token" -AsSecureString
$RedisUrl = Read-Host "Shared Redis URL"
if ([string]::IsNullOrWhiteSpace($ApiUrl) -or [string]::IsNullOrWhiteSpace($WorkerId) -or [string]::IsNullOrWhiteSpace($RedisUrl)) { throw "API URL, worker ID, and Redis URL are required." }
$WorkerToken = [System.Net.NetworkCredential]::new("", $WorkerTokenSecure).Password
if ([string]::IsNullOrWhiteSpace($WorkerToken)) { throw "Worker token is required." }
...
Pop-Location
Write-Host "DeployPilot worker installed and started."
