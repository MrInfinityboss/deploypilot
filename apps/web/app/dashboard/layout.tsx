...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const update = () => { const visibleWidth = window.visualViewport?.width ?? window.innerWidth; const actualWindowWidth = window.outerWidth || visibleWidth; const narrowPane = visibleWidth < window.innerHeight * 1.35; setCompact(actualWindowWidth <= 1050 || visibleWidth <= 1050 || narrowPane); }; update(); window.addEventListener("resize", update); window.visualViewport?.addEventListener("resize", update); return () => { window.removeEventListener("resize", update); window.visualViewport?.removeEventListener("resize", update); }; }, []);
