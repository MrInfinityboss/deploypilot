...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="dp-shell" style={{ minHeight:"100vh", display:"grid", gridTemplateColumns:"232px 1fr" }}>
    <aside className={`dp-sidebar${sidebarOpen ? " dp-sidebar-open" : ""}`} style={{ borderRight:"1px solid var(--line)", background:"#0d1016", padding:"25px 14px", position:"sticky", top:0, height:"100vh", display:"flex", flexDirection:"column" }}>
    ...
    <main className="dp-main" style={{ minWidth:0, padding:"30px 38px 60px", maxWidth:1500, width:"100%" }}><div className="dp-mobile-bar"><button className="dp-menu-button" aria-label="Open navigation" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(value => !value)}><span /><span /><span /></button><span style={{ fontWeight:800, letterSpacing:"-.04em" }}>DeployPilot</span></div>{sidebarOpen && <button className="dp-sidebar-backdrop" aria-label="Close navigation" onClick={() => setSidebarOpen(false)} /> }<WorkspaceProvider>{children}</WorkspaceProvider></main>
  </div>;
}
